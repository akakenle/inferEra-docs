...
点击代码块右上角「Copy」可复制请求命令，部分环境支持「Try it out」直接发起请求（需提前配置 OpenAPI 规范）。

<OpenAPISnippet method="GET" url="https://aihubmix.com/api/v1/models?type=llm">
  获取所有大语言模型
</OpenAPISnippet>

<OpenAPISnippet method="GET" url="https://aihubmix.com/api/v1/models?sort_by=coding&sort_order=desc">
  获取适合编程的模型，按上下文长度排序
</OpenAPISnippet>

<OpenAPISnippet method="GET" url="https://aihubmix.com/api/v1/models?model=gpt-5">
  搜索特定模型（GPT-5）
</OpenAPISnippet>

<OpenAPISnippet method="GET" url="https://aihubmix.com/api/v1/models?type=llm&modalities=text,image&features=function_calling&sort_by=model_ratio&sort_order=asc">
  复合条件筛选（LLM+图文模态+函数调用+高性价比）
</OpenAPISnippet>

<OpenAPISnippet method="GET" url="https://aihubmix.com/api/v1/models?sort_by=coding">
  编程模型智能排序（优先展示编程友好型模型）
</OpenAPISnippet>
