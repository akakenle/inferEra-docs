...
<ResponseField name="types" type="string">
  模型类型。支持值：`llm`、`image_generation`、`video`、`tts`、`stt`、`embedding`、`rerank`，多个类型用逗号分隔
</ResponseField>

<ResponseField name="features" type="string">
  支持的功能特性。支持值：`thinking`（支持思考推理）、`tools`（支持工具调用）、`function_calling`（支持函数调用）、`web`（支持搜索）、`deepsearch`（支持深度搜索）、`long_context`（长上下文模型）、`structured_outputs`（结构化输出），多个特性用逗号分隔
</ResponseField>

<ResponseField name="input_modalities" type="string">
  支持的输入模态。支持值：`text`、`image`、`audio` 、`video`、`pdf`，多个模态用逗号分隔
</ResponseField>
...
<ParamField query="type" path="string" type="string">
  模型类型。支持值：`llm`、`image_generation`、`video`、`tts`、`stt`、`embedding`、`rerank`
</ParamField>

<ParamField query="modalities" path="param" type="string">
  输入模态。支持值：`text`、`image`、`audio` 、`video`、`pdf`，支持多模态查询（逗号分隔）
</ParamField>
