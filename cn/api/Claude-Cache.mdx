...
client = anthropic.Anthropic(
    api_key=os.getenv("AIHUBMIX_API_KEY"),  # 或直接使用 "sk-***"
...
| 模型 | 基础输入 Token | 5分钟缓存写入 | 1小时缓存写入 | 缓存命中和刷新 | 输出 Token |
|------|---------------|-------------|-------------|--------------|-----------|
...
注意：
- 5分钟缓存写入 Token 价格为基础输入 Token 价格的 1.25 倍
- 1小时缓存写入 Token 价格为基础输入 Token 价格的 2 倍
...
- Claude 需要原生的 `cache_control: { type: "ephemeral" }` 声明，缓存费率为常规输入的 1.25 倍（5分钟）或 2 倍（1小时），缓存 Tokens 读取费用为 0.1 倍，生命周期 5 分钟或 1 小时。[文档](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching#how-to-implement-prompt-caching)
