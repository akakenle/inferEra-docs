---
title: "OpenAI Codex CLI 集成"
description: "通过 AiHubMix 在命AiHubMix 提供了与 OpenAI Codex CLI 的无缝集成，让你可以在命令行环境中利用先进的 AI 编程助手。通过简单的配置步骤，即可在终端中直接使用自然语言执行各种编程和系统操作任务。"
icon: "terminal"
---
...
\*\*打开你的 Shell 配置文件（例如 ****``.zshrc或 ** .bashrc），添加以下环境变量：``****
...
<Warning>
  \*\*这里的 [******`OPENAI_API_KEY应该使用 AiHubMix 的 密钥`******](https://aihubmix.com/token)，变量名保持为 \*\*`OPENAI_API_KEY`**是为了兼容 OpenAI 原生客户端。。**
</Warning>
