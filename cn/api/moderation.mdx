...
- **Endpoint (API 地址)**: `https://api.aihubmix.com/v1/moderations`
- **请求方式**: `POST`
...
| **输出类别**                       | **描述**                                                                                                                          |
| :----------------------------- | :------------------------------------------------------------------------------------------------------------------------------ |
| `flagged`                      | `true`如果模型将内容分类为潜在有害内容，则设置为 true；`false`否则设置为 false。                                                                            |
| `categories`                   | 包含一个字典，其中列出了每个类别对应的违规标志。对于每个类别，该值表示`true`模型是否将相应类别标记为违规，`false`否则为空。                                                            |
| `category_scores`              | 包含模型输出的按类别划分的分数字典，表示模型对输入违反 OpenAI 该类别策略的置信度。该值介于 0 和 1 之间，值越高表示置信度越高。                                                          |
| `category_applied_input_types` | 此属性包含有关响应中标记了哪些输入类型的信息，并按类别进行划分。例如，如果模型中的图像和文本输入都被标记为“暴力/画面”，则此`violence/graphic`属性将被设置为 true `["image", "text"]`。此功能仅适用于全渠道模型。 |
