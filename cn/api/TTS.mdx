...
#### 标准 TTS 参数（适用于 tts-1, tts-1-hd, gpt-4o-mini-tts, gemini-2.5-flash-preview-tts, gemini-2.5-pro-preview-tts）
...
| response_format | string | 否  | 音频输出格式。支持的音频格式包括： `mp3`、 `opus`、 `aac` 、 `flac`、 `wav`、 `pcm`，默认使用  `mp3` 格式。\*\*注意：Gemini 模型仅支持 \*\*`wav`\*\*和 \*\* `pcm`**格式。** |
