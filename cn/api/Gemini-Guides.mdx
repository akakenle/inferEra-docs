...
<Tip>
  ...
  2. Gemini 2.5 Flash 通过 `budget`（思考预算）来控制思考的深度，范围 0-24K，目前转发采用的是默认预算 1024，最佳边际效果为 16K。
  3. 只能通过 http 请求入参 `"reasoning": { "enabled": true, "budget": 1024 }` 来显示 thinking 内容（`reasoning_content`）
</Tip>
