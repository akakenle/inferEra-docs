...
使用前运行 `pip install google-genai` 或 `pip install -U google-genai`，按照（更新）原生依赖。

对于原生的转发支持，主要是在内部传入 AiHubMix 密钥和请求链接。需要注意的是，这个链接和常规的 `base_url` 写法不同，请参考示例： 
...
Openai 兼容方式调用参考如下。用于快速任务时，以下调用展示了**关闭思考**的方法。
