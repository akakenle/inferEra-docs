...
- 缓存有效期（TTL）由开发者设定，也可以不设置。如果未指定，默认为 1 小时。无最小或最大时长限制，费用取决于缓存 token 数与缓存时间。
  - 虽然 Google 官方对 TTL 不设上下限，但由于我们作为转发平台，**仅支持有限的 TTL 配置范围**。

### 注意事项

- **无成本节省保证**：缓存 token 的计费为输入原价的 25%，理论上输入部分可最多节省 75% 成本，**但 [Google 官方](https://ai.google.dev/gemini-api/docs/caching?lang=python)并未承诺必然节省**，实际账单还需结合缓存命中率、token 类型与存储时长共同评估。
