...
<Note>
  图生视频需通过 `input_reference` 参数传入参考图片（阿里通义万相）；豆包 Seedance 通过 `extra_body.content` 数组传入，支持图片、视频、音频多种参考类型。
</Note>
...
```shellscript Seedance 2.0 / 2.0 Fast
curl -X POST "https://aihubmix.com/v1/videos" \
  -H "Authorization: Bearer $AIHUBMIX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
