...
### Why does my account still fail to call certain models (such as Claude Opus or GPT-5.4-Pro) even though it has a balance?

We use a **pre-charge mechanism**. When calling higher-cost models (such as Claude Opus or GPT-5.4-Pro), the system estimates the maximum possible output cost based on the configured output token limit and checks your balance when the request is initiated. If your account balance is **less than the output token price × maximum output tokens**, the request may be rejected. To avoid this, ensure your account balance is sufficient before calling these models and set a reasonable maximum output token limit; if not specified, the system will use the default value for estimation. Note that the parameter name for maximum output differs by model: Claude and GPT models use `max_tokens`, while Gemini models use `max_output_tokens`.
...
### Why Do Responses from the Claude Model End Prematurely?

For the Claude model, AIHubMix currently supports two calling methods:

1. OpenAI Chat compatible interface
2. Anthropic Claude native interface

When invoking the Claude model through the OpenAI Chat compatible interface, the system defaults to max_tokens=4096. If a larger max_tokens is not explicitly set in the request, the model will automatically stop outputting once it reaches this limit. Therefore, the occurrence of "responses ending before completion" is usually not due to a model malfunction, but rather because it has hit the default output length restriction.

**How to Generate Longer Texts?**

Under the OpenAI Chat compatible interface, you can manually set a larger max_tokens parameter, for example:
...
max_tokens must not exceed the maximum supported limit for the corresponding model. If truncation still occurs after setting it, please provide the model name and the complete request parameters for further investigation.
