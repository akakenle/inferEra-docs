...
We recently introduced a pre-charge mechanism. When high-concurrency requests are made to higher-cost models (such as the Claude Opus series or GPT-5.4-Pro series), the system performs a pre-charge validation based on the maximum possible output cost at the time the request is initiated.
