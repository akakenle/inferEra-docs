...
<ParamField body="model" type="string" required>
  ...
  - `jina-clip-v1`：Multi-modal model, supports images and English text, 768-dimensional, 8K context window, 223M parameters
  - `jina-colbert-v1-en`：English ColBERT model, 8K token context, 137M parameters, used for embedding and reranking
  - `jina-embeddings-v2-base-es`：Spanish-English bilingual embedding model, 768-dimensional, 8K context window, 161M parameters
  ...
  - `jina-embeddings-v2-base-de`：German-English bilingual embedding model, 768-dimensional, 8K context window, 161M parameters
  - `jina-embeddings-v2-base-zh`：Chinese-English bilingual embedding model, 768-dimensional, 8K context window, 161M parameters
  - `jina-embeddings-v2-base-en`：English embedding model, comparable to OpenAI's text-embedding-ada002, 768-dimensional, 8K context window, 137M parameters
</ParamField>
...
<ParamField body="model" type="string" required>
  Model name, available model list:
  - `jina-reranker-v2-base-multilingual`：The latest multilingual reranker, 278M parameters, 8K context
  - `jina-reranker-m0`：Multimodal multilingual document reranker, 10K context, 2.4B parameters, for visual document sorting
  - `jina-colbert-v2`：Multilingual ColBERT model, 8K context, 560M parameters, for embedding and reranking
  - `jina-reranker-v1-tiny-en`：The fastest reranking model, suitable for reliably sorting a large number of documents, 8K context, 33M parameters
  - `jina-reranker-v1-turbo-en`：The best combination of fast inference speed and accurate relevance scores, 8K context, 37.8M parameters
  - `jina-reranker-v1-base-en`：Our first reranking model, maximizing search and RAG relevance, 8K context, 137M parameters
  - `jina-colbert-v1-en`：English ColBERT model, 8K context, 137M parameters
</ParamField>
...
```json
{
  ...
  "content": [
    ...
    {
      "type": "image",
      "image": "data:image/webp;base64,UklGRoDOAAB...", // base64 encoding of images
      "mimeType": "image/webp"
    }
  ]
}
