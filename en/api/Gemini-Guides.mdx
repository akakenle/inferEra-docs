...
<Tip>
  <strong>Streaming Mode (`stream=True`)</strong> → reasoning stage only  
  <strong>Non-Streaming Mode (`stream=False`)</strong> → final image generation

  Generated images are <strong>not included in streaming responses</strong> and must be retrieved using a <strong>non-streaming request</strong>.
</Tip>
