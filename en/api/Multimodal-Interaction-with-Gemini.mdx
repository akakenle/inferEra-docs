...
Imagen 3.0 is an advanced image generation AI model launched by Google, capable of creating high-quality, realistic images based on text prompts. This guide will help you understand how to use the Imagen 3.0 API to generate images, including parameter settings, model selection, and code examples.

<Tip>
1. Currently, Imagen 3.0 only supports English prompts. When integrating, it's recommended to add automatic translation to allow users to use it without language barriers.
...
The cost of using the Imagen 3.0 API to generate images is **$0.03 per image**. Please note that each API call can generate 1-4 images, and you will be charged based on the actual number of images generated.
...
Gemini also offers image generation capabilities as an alternative. Compared to Imagen 3.0, Gemini's image generation is better suited for scenarios that require contextual understanding and reasoning, rather than pursuing ultimate artistic expression and visual quality.
...
client = genai.Client(
    api_key=os.getenv("AIHUBMIX_API_KEY"), # 🔑 Replace with your AiHubMix API key
    http_options={"base_url": "https://aihubmix.com/gemini"},
)
